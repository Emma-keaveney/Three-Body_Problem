...
float accel(float x, float y,float x_c,float y_c, float x_p, float y_p, float M, float m_p, float e, bool x_dir){
    float D_C = sqrt(pow(x,2) + pow(y,2)+ pow(e,2));
    float D_P = sqrt(pow(x-x_p,2) + pow(y-y_p,2) + pow(e,2));

    float acceleration;

    if (x_dir == true){
        acceleration = -G*M*x/pow(D_C,3) - G*m_p*(x-x_p)/pow(D_P,3);
    }
    else{
        acceleration = -G*M*y/pow(D_C,3) - G*m_p*(y-y_p)/pow(D_P,3);
    }
    return acceleration;
}
...
/**
 * 
 * @brief Uses the leapfrog method to find the new position and speed of a star at each time-step
 * 
 * @param x: x coordinate of the star
 * @param y: y coordinate of the star
 * @param x_c: x coordinate of the galaxy centre of mass
...
/**
 * 
 * @brief At each time-step calculates the new coordinates and velocities of each star using the function 
 * taylor_one_step, then recalculates the position of the barycentre of the galaxy. Assumes the perturber 
 * moves linearly, feeling no force from the galaxy, and the galaxy is initially stationary in the 
 * reference frame.
 * 
 ...
 * @param v_p: velocity of the perturber
...
> 
taylor_all_stars(std::vector<float> x,std::vector<float> y, std::vector<float> v_x, std::vector<float> v_y, float v_p, std::vector<float> m, float m_p, float a, float e, int n, std::vector<float> initial, bool leapfrog){
 ...
 * 
 * This is done by modelling the initial positions of the stars uniformly between 0.1 and R, the 
 * radius of the galaxy. The masses are randomly distributed, and the initial circular velocities found 
 * using v = (GM/r)^(1/2) where M is the total mass of the galaxy. From the circular velocities of the 
 * stars, the orbital period can be found. The softening constant e = R_max p^(-1/2) is calculated.

 * @param p: number of stars
 * @param r_max: radius of the galaxy in kiloparsecs
 * @param m_min: minimum stellar mass of a star in the galaxy in solar masses
 * @param m_max: maximum stellar mass of a star in the galaxy in solar masses
 * 
 ...
 * constant, the shortest andlongest orbital periods of stars in the galaxy, and the speed of the slowest
...
>
initialise_values(int p, float r_max, float m_min, float m_max){

    std::vector<float> r = linspace(0.1, r_max, p);
    std::vector<float> theta = random_number(0, 2*pi, p);
    std::vector<float> m = random_number(m_min, m_max, p);

    float r_max_val = *std::max_element(r.begin(), r.end());
    float r_min_val = *std::min_element(r.begin(), r.end());

    float e = r_max_val/sqrt(p);    //define the softening constant
...
int main(){
    ...
    int p = 200;    //number of stars
    float mu = 2;    //mass ratio of perturber to galaxy
    float V = 0.8;   //ratio of perturber velocity to slowest star's circular velocity
    float r_max = 5;    //radius of galaxy
    float m_min = 0.1;    // mass of smallest star in galaxy 
    float m_max = 5;     // mass of largest star in galaxy
    int n = 70000;    //number of time steps
    std::vector<float> initial = {-20, 0};

    auto values = initialise_values(p, r_max, m_min, m_max);

    float v_p = V*std::get<9>(values);
    std::cout << "Perturber speed is " << v_p << std::endl;

    // set the time period of the simulation to be twice the time taken for the perturber to reach the galaxy 
    float T = 2*sqrt(pow(initial[0],2)+pow(initial[1],2))/v_p;
    ...
    auto results = taylor_all_stars(std::get<0>(values),std::get<1>(values), std::get<2>(values), std::get<3>(values), v_p, std::get<4>(values), m_p, a, std::get<6>(values), n, initial, true);
